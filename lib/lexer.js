function lexer (codeArray, pos, lineno, colno) {
  let tree = [],
      token = "",
      ch,
      isEsc= false,
  ...
  while (pos < codeArray.length) {
    ...
    ++pos;
    //newline
    if (c === "\n") { if (isCmt) {isCmt=false;} ++lineno; colno=1; }
    if (isCmt) { continue; }
    //inside string?
    if (isEsc) { isEsc=false; token += c; continue; }
    // strings
    if (c === '"') { isStr = !isStr; token += c; continue; }
    if (isStr) {
      if (c === "\n") { token += "\\n"; }
      else { if (c === "\\") { isEsc=true; } token += c; }
      continue;
    }
    // single string
    if (c === "'") { isSQStr = !isSQStr; token += c; continue; }
    if (isSQStr) { token += c; continue; }
    // data types
    if (c === "[") {

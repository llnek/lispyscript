...
keywords["set"] = function(form) {
  ...
  return snode(null, null, null,
               [form[1],
                " = ",
                (isarray(form[2]) ? evalForm(form[2]) : form[2])]);
};

keywords["aset"] = function(form) {
  if (form.length < 3 || form.length > 4) { synError(0, form); }
  if (form.length === 4) {
    form[1] = (isform(form[2]) ? evalForm(form[2]) : form[2]) +
             "[" + form[1] + "]";
